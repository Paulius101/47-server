...
handlers.cars.get = async (data, callback) => {
    // gaunam car info
...
handlers.cars.post = async (data, callback) => {
    // irasom car info
...
handlers.cars.put = async (data, callback) => {
    // atnaujinam car info
    const { VIN, model, color } = data.payload;
    ...
    const content = await _data.read('car', VIN);
    if (content === '') {
        return callback(400, {
...
handlers.cars.delete = async (data, callback) => {
    // istrinam user info
